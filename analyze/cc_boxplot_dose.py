...
def create_boxplots(ax, data, methods, title):
    ...
    custom_labels = ['Pyradiomics', 'Shallow CNN', 'SwinUNETR', 'SwinUNETR v2']
    ...
    with open(stats_file_path, 'w') as file:
        for method in methods:
            ...
            if values:
                median = np.median(values)
                std = np.std(values)
                file.write(f"{method}: Median={median:.4f}, Std={std:.4f}\n")
    print(f"Merged median and std saved to {stats_file_path}")
...
def create_merged_boxplot(all_data, methods, scanners, output_dir):
    ...
    box = ax.boxplot(plot_data, labels=['Pyradiomics', 'Shallow CNN', 'SwinUNETR', 'SwinUNETR v2'], showfliers=False)
    ...
    ax.set_title("ICC Comparison Across Methods (All Scanners Merged)", fontsize=14, fontweight='bold', pad=25)
...
def main():
    ...
    methods = ['pyradiomics', 'cnn', 'swinunetr', 'swinunetr2']
    ...
    for method in methods:
        for scanner in scanners:
            if method == 'swinunetr2':
                file_path = f'{files_dir}/icc_scanner_{scanner}/icc_dose_features_swinunetr_reversed_{scanner}.csv'
            else:
