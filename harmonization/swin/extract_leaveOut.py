...
def run_inference(model,jsonpath = "./expanded_registered_light_dataset_info.json",fname = ""):
    ...
    slice_num = 15
    os.makedirs(f"./features_loso", exist_ok=True)
    with open(f"./features_loso/features_{os.path.basename(fname)}.csv", "w", newline="") as csvfile:
...
def main():
    ...
    fnames = [item.replace('.pth', '') for item in fnames]
    for fname in fnames:
