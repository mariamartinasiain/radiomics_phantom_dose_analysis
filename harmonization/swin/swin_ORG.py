...
def crop_centered_patch(tensor: torch.Tensor, patch_size=(64, 64, 32)) -> torch.Tensor:
    """
    Crops a 3D tensor to a fixed-size patch centered at the non-zero region.

    Input tensor shape: [C, D, H, W]
    Patch size: (depth, height, width)

    Returns a cropped tensor of shape [C, patch_size[0], patch_size[1], patch_size[2]]
    """
    assert tensor.dim() == 4, "Expected tensor of shape [C, D, H, W]"

    nonzero = tensor.sum(dim=0).nonzero(as_tuple=False)
    if nonzero.size(0) == 0:
        return tensor[:, :patch_size[0], :patch_size[1], :patch_size[2]]  # fallback crop

    center_dhw = nonzero.float().mean(dim=0).round().long()
    center_d, center_h, center_w = center_dhw.tolist()
    ...
    return tensor[:, d_start:d_end, h_start:h_end, w_start:w_end]

def crop_centered_patch_from_mask(
    tensor: torch.Tensor, 
    mask: torch.Tensor, 
    patch_size=(128, 128, 48)
) -> torch.Tensor:
    """
    Crops a 3D tensor to a fixed-size patch centered at the non-zero region of the mask.

    Parameters:
    - tensor: [C, D, H, W] input image
    - mask: [C, D, H, W] binary mask used to compute the center
    - patch_size: (D, H, W) of the cropped patch

    Returns:
    - Cropped tensor of shape [C, patch_size[0], patch_size[1], patch_size[2]]
    """
    assert tensor.dim() == 4 and mask.dim() == 4, "Expected tensors of shape [C, D, H, W]"

    nonzero = mask.sum(dim=0).nonzero(as_tuple=False)
    if nonzero.size(0) == 0:
        # fallback crop from corner if no non-zero region in mask
        return tensor[:, :patch_size[0], :patch_size[1], :patch_size[2]]

    center_d, center_h, center_w = nonzero.float().mean(dim=0).round().long().tolist()
    pd, ph, pw = patch_size
...
def run_inference(nifti_dir, masks_dir, output_dir):
    ...
    output_csv = os.path.join(output_dir, "features_swinunetr_org_v2.csv")
            ...
            for label, name in label_to_name.items():
                ...
                for patch_tensor, patch_name in patches:
                    ...
                    with torch.no_grad():
                        ...
                        val_outputs = model.swinViT(patch_tensor.float())
...
if __name__ == "__main__":
    ...
    output_dir = "/mnt/nas7/data/maria/final_features/swinunetr/ct-org"
    os.makedirs(output_dir, exist_ok=True)
...
# Carga tu CSV
csv_path = '/mnt/nas7/data/maria/final_features/swinunetr/ct-org/features_swinunetr_org_v2.csv'
