...
centersrois = {
    "normal": [226, 161, 149],
    "cyst": [179, 324, 157],
    "hemangioma": [193, 212, 159],
    "metastasis": [240, 111, 140],
    "spleen": [322, 400, 159],
    "right kidney": [337, 192, 131],
    "pancreas": [252, 343, 129],
    "L1 vertebra": [326, 267, 142]
}
centersrois = json.load(open("/home/reza/radiomics_phantom/centersrois_10.json"))
path_size = (64, 64, 32)  # Patch size (depth, height, width)
...
def run_inference():
    ...
    datafiles = sorted([f for f in os.listdir(nifti_dir) if f.endswith(".nii.gz")])#[:5]

    output_dir = "/mnt/nas7/data/maria/final_features/"
    output_file = os.path.join(output_dir, "swinunetr_features_not_registered_6rois_10regions_test.csv")
    ...
    with open(output_file, "w", newline="") as csvfile:
        ...
        for file in tqdm(datafiles, desc="Processing CT scans"):
            ...
            for roi_label, center in centersrois.items():
                ...
                patch = extract_patch(image, patch_center, patch_size=path_size)
