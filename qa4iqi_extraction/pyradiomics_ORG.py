...
def align_mask_to_image(image_nii, mask_nii, mask_path):
    ...
    if not np.array_equal(image_affine[:3, 3], mask_affine[:3, 3]):
        ...
        #nib.save(mask_nii_aligned, mask_path)
        print(f"Mask {os.path.basename(mask_path)} affine adjusted to match image affine.")
...
for file in tqdm(nifti_files, desc="Processing CT scans"):
    ...
    for label, name in label_to_name_v2.items():
        ...
        mask_paths_for_file.append(mask_path)
...
# plt.legend()  # Puedes activarlo si quieres
plt.grid(True)

# Guarda la imagen
output_path = os.path.join(os.path.dirname(csv_path), 'pyradiomics_org_umap_final.png')
